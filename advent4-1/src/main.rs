...
fn main() {
    ...
    let mut winner_board_index: i32 = -1;
    ...
    for drawn_number in drawn_numbers {
        for (index, board) in boards.iter_mut().enumerate() {
            ...
            if board.is_winner() {
                winner_board_index = index as i32;
                winning_drawn_number = drawn_number;
            }
        }
    }

    let sum_of_unmarked_numbers_winner_board =
        boards[winner_board_index as usize].sum_of_unmarked_numbers_in_winner_board();
